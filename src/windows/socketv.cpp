...
namespace SV {
    ...
    listener socketv::listen(ConnType connection_type, address addr) {
        ...
        if (l.socket == INVALID_SOCKET) {
            throw exception("SV::listen", "invalid socket", WSAGetLastError(), get_last_wsa_error_message());
        }
        ...
        if (addr.address_ip_type == ipv4) {
            ...
            if (::bind(l.socket, (sockaddr *)&sin, sizeof(sin)) == SOCKET_ERROR) {
                throw exception("SV::listen", "bind error", WSAGetLastError(), get_last_wsa_error_message());
            }
        ...
        if (connection_type == tcp) {
            if (::listen(l.socket, backlog) == SOCKET_ERROR) {
                throw exception("SV::listen", "listen error", WSAGetLastError(), get_last_wsa_error_message());
            }
        }
    ...
    sender socketv::connect(ConnType connection_type, address local_addr, address remote_addr) {
        ...
        if (c.socket == INVALID_SOCKET) {
            throw exception("SV::connect", "invalid socket", WSAGetLastError(), get_last_wsa_error_message());
        }
        ...
        if (local_addr.address_ip_type == ipv4) {
            ...
            if (::bind(c.socket, (sockaddr *)&sin, sizeof(sin)) == SOCKET_ERROR) {
                throw exception("SV::connect", "bind error", WSAGetLastError(), get_last_wsa_error_message());
            }
        ...
        if (connection_type == tcp) {
            if (remote_addr.address_ip_type == ipv4) {
                ...
                if (::connect(c.socket, (sockaddr *)&sin, sizeof(sin)) == SOCKET_ERROR) {
                    throw exception("SV::connect", "connect error", WSAGetLastError(), get_last_wsa_error_message());
                }
