...
namespace SV {
    sender listener::accept() {
        ...
        if (local_addr.address_ip_type == ipv4) {
            ...
            if (c.socket == INVALID_SOCKET) {
                throw exception("SV::listener::accept", "accept error", WSAGetLastError(), get_last_wsa_error_message());
            }
    ...
    int listener::sendto(address addr, char *data, int length) {
        ...
        if (sended_size == SOCKET_ERROR) {
            throw exception("SV::listener::sendto", "sendto error", WSAGetLastError(), get_last_wsa_error_message());
        }
        return sended_size;
    }
    ...
    int listener::recvfrom(address *addr, char *buffer, int buffer_size) {
        ...
        if (local_addr.address_ip_type == ipv4) {
            ...
            if (recv_size == SOCKET_ERROR) {
                throw exception("SV::listener::recvfrom", "recvfrom error", WSAGetLastError(), get_last_wsa_error_message());
            }
