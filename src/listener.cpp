...
namespace SV {
    sender listener::accept() {
        ...
        if (local_addr.address_ip_type == ipv4) {
            ...
            socketv_socklen addr_len = sizeof(remote_sin);
            c.socket = ::accept(socket, (socketv_sockaddr *)&remote_sin, &addr_len);
            if (c.socket == SOCKETV_INVALID_SOCKET) {
                throw exception("SV::listener::accept", "accept error", get_last_error_code(), get_last_error_message());
            }
    ...
    int listener::sendto(address addr, char *data, int length) {
        ...
        if (sended_size == SOCKETV_SOCKET_ERROR) {
            throw exception("SV::listener::sendto", "sendto error", get_last_error_code(), get_last_error_message());
        }
        return sended_size;
    }
    ...
    int listener::recvfrom(address *addr, char *buffer, int buffer_size) {
        ...
        if (local_addr.address_ip_type == ipv4) {
            ...
            if (recv_size == SOCKETV_SOCKET_ERROR) {
                throw exception("SV::listener::recvfrom", "recvfrom error", get_last_error_code(), get_last_error_message());
            }
